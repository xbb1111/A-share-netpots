...
import { getComputableBranchStocks, type IndustryIndexPreviewRequest } from '../data/industryIndexPreview';
...
export function createIndustryPreviewRequest(node: CanvasNode, method: IndustryIndexPreviewRequest['method'], sourcePath: string[]) {
  const totalCompanyCount = getComputableBranchStocks(node).length;
...
export function getIndustryPreviewAvailability(companies: IndustryCompany[], loading: boolean) {
  const validCodes = new Set(companies.map((company) => company.code.trim()).filter((code) => /^\d{6}$/.test(code)));
  if (loading) return { disabled: true, label: '加载公司中…', message: '' };
  return validCodes.size === 0
    ? { disabled: true, label: '预览行业指数', message: '当前标签暂无可计算公司' }
    : { disabled: false, label: '预览行业指数', message: '' };
}

export function getCanvasPreviewSourcePath(canvasName: string, path: CanvasNode[]) {
  const names = path.map((item) => item.name);
  return names[0] === canvasName ? names : [canvasName, ...names];
}
...
export function IndustriesPage({ industries, onOpenPriceTool, onPreviewIndex, onOpenIndexTool }: { industries: IndustryBoard[]; onOpenPriceTool?: (code: string, name: string) => void; onPreviewIndex?: (request: IndustryIndexPreviewRequest) => void; onOpenIndexTool?: (node: CanvasNode, method: 'equal' | 'marketCap') => void }) {
  ...
  function activateMarketBoard(board: IndustryBoard) {
    const chainRoute = findChainRouteForBoard(board, INDUSTRY_CHAINS);
    if (chainRoute) {
      setChainId(chainRoute.chainId);
      setSelectedNodeId(chainRoute.nodeId);
      setSelectedBoardCode(null);
      setView('chain');
    } else {
      setSelectedBoardCode(board.code);
      setView('panorama');
    }
  }
    ...
    <section className="panel industry-workbench" id="industries">
      ...
      {view === 'market' ? (
        <div className="industry-market-view">
          ...
          <IndustryMarketMap boards={industries} metric={cloudMetric} onActivate={activateMarketBoard} />
        </div>
      ) : null}
