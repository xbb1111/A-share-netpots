...
console.log('loading stock universe and themes');
...
const output = `// Generated by scripts/build-sentiment-seed.mjs from public market sources.\nexport const SENTIMENT_SEED_ROWS = ${JSON.stringify(seedRows)};\nexport const SENTIMENT_SEED_META = ${JSON.stringify({ generatedAt: new Date().toISOString(), universeSize: metadata.length, commonAsOf: seedRows.at(-1)?.date, marketAsOf: targetDates.at(-1), dataLagTradingDays: Math.max(0, targetDates.length - 1 - targetDates.indexOf(seedRows.at(-1)?.date)), industryClassification: '申万一级（由东方财富申万二级标签映射）', industryMappingCoverage: round(mappingProfile.coverage * 100, 2), themeAsOf: themeSnapshot.asOf, topThemes: themeSnapshot.items, note: '公开数据代理口径；行业成交额按申万一级互斥归类，主题榜独立展示且不计入占比；历史 ERP 使用国证A指点位比例回推市盈率。' })};\n`;
...
async function fetchStockUniverse() {
  ...
  return [first, ...rest]
    ...
    .map((row) => ({ code: String(row.f12), industryLevelTwo: String(row.f100 ?? '').trim(), industry: toLevelOneIndustry(row.f100), freeMarketCap: number(row.f21) }));
}
