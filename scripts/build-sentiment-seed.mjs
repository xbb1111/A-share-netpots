...
await concurrentMap(metadata, CONCURRENCY, async (stock) => {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (Number.isFinite(row.changePercent)) {
      ...
      if (row.changePercent > 0) aggregate.rising += 1;
    }
...
const seedRows = targetDates.map((date) => {
  ...
  const industryAmounts = [...aggregate.industryAmounts.values()].sort((a, b) => b - a);
  const industryTotal = industryAmounts.reduce((sum, value) => sum + value, 0);
  return {
    date,
    turnover: round(aggregate.turnoverWeighted / aggregate.turnoverWeight, 4),
    top3IndustryShare: round((industryAmounts.slice(0, 3).reduce((sum, value) => sum + value, 0) / industryTotal) * 100, 4),
...
}).filter((row) => Object.values(row).slice(1).every(Number.isFinite));
...
function createAggregate() {
  return { turnoverWeighted: 0, turnoverWeight: 0, totalAmount: 0, industryAmounts: new Map(), rising: 0, risingValid: 0, aboveMa20: 0, ma20Valid: 0 };
}
