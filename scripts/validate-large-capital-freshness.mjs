...
if (!asOf || lagDays > maxLagDays || etfCount < 8 || productCount < 3) {
  throw new Error(`Large-capital snapshot failed validation: asOf=${LARGE_CAPITAL_SEED.asOf}, lagDays=${lagDays.toFixed(1)}, etfs=${etfCount}, products=${productCount}`);
}

console.log(`Large-capital snapshot valid: ${LARGE_CAPITAL_SEED.asOf}, ${etfCount} ETFs, ${productCount} products`);
